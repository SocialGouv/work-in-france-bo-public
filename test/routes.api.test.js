...
describe("GET /api/v1/apt_validity_check/:ds_id/:date", () => {
  ...
  it("valid APT in another file", done => {
    chai
      .request(server)
      .get("/api/v1/apt_validity_check/999999/1981-01-20")
      .end((err, res) => {
        should.not.exist(err);
        res.status.should.equal(200);
        res.type.should.equal("application/json");
  ...
  it("outdated APT should be invalid", done => {
    chai
      .request(server)
      .get("/api/v1/apt_validity_check/777777/1992-06-12")
      .end((err, res) => {
        should.not.exist(err);
        res.status.should.equal(200);
        res.type.should.equal("application/json");
